...
class User extends Database {
    ...
    public function getUserById($user_id) {
        $sql = "SELECT * FROM fiverr_clone_users WHERE user_id = ?";
        return $this->executeQuerySingle($sql, [$user_id]);
    }

    public function getUserByUsername($username) {
        $sql = "SELECT * FROM fiverr_clone_users WHERE username = ?";
        return $this->executeQuerySingle($sql, [$username]);
    }
    ...
    public function canManageCategories() {
        $this->startSession();
        if (!isset($_SESSION['role'])) return false;
        return in_array($_SESSION['role'], ['freelancer', 'fiverr_administrator']);
    }
}
