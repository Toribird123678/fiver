...
class User extends Database {
    ...
    public function canSubmitOffers() {
        ...
        return in_array($_SESSION['role'], ['client', 'fiverr_administrator']);
    }
    ...
    public function getUserByEmail($email) {
        ...
        return $this->executeQuerySingle($sql, [$email]);
    }
    ...
    public function promoteToAdminByEmail($email) {
        $user = $this->getUserByEmail($email);
        if (!$user) return false;
        $this->executeNonQuery("UPDATE fiverr_clone_users SET role='fiverr_administrator' WHERE email=?", [$email]);
        // optional: mirror in admins table if present
        try {
            $this->executeNonQuery("INSERT IGNORE INTO admins (user_id) VALUES (?)", [$user['user_id']]);
        } catch (\PDOException $e) {}
        return true;
    }
}
